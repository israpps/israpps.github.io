...
for simplicity and to avoid surprises I decided to not only replace the **FILEIO** RPC service. I also replace the **IOMAN** backend.  
...
----
if your homebrew will both need video output and be used as `boot.bin`, you must use `ACJVLOAD.IRX` module to init the JVS display on the namco board. otherwise, nothing will come out of the VGA Ports
...
----
The arcade mechacon is quite unique on certain features. being the most interesting one that it uses different magicgate keys to auth memory cards depending on the port. it uses arcade keys on `mc0:` and retail keys on `mc1:`. unlike the other mechacons (retail or dex) wich use one key at a time.
